...
/// Ultron'".
///
/// Each intent routes to a `ultron://` URL so the existing URL handler owns
/// the actual execution path — one code path, two entry points.

@available(macOS 13.0, *)
struct AskUltronIntent: AppIntent {
    static let title: LocalizedStringResource = "Ask Ultron"
    static let description = IntentDescription("Stil Ultron et spørgsmål og få et kildebaseret svar.")
    static let openAppWhenRun: Bool = true

    @Parameter(title: "Prompt", description: "Det du vil spørge Ultron om.")
    var prompt: String

    func perform() async throws -> some IntentResult {
        try await openUltron(action: "qna", prompt: prompt)
        return .result()
    }
}

@available(macOS 13.0, *)
struct OpenUltronChatIntent: AppIntent {
    static let title: LocalizedStringResource = "Open Ultron Chat"
    static let description = IntentDescription("Åbn Ultron-chatvinduet og forudfyld evt. prompt-feltet.")
    static let openAppWhenRun: Bool = true
    ...
    func perform() async throws -> some IntentResult {
        try await openUltron(action: "chat", prompt: prompt)
        return .result()
    }
}

@available(macOS 13.0, *)
struct VisionInUltronIntent: AppIntent {
    static let title: LocalizedStringResource = "Ask Ultron about screen"
    static let description = IntentDescription("Tag et skærmbillede og stil Ultron et spørgsmål om det.")
    static let openAppWhenRun: Bool = true
    ...
    func perform() async throws -> some IntentResult {
        try await openUltron(action: "vision", prompt: prompt)
        return .result()
    }
}
...
struct OpenCockpitIntent: AppIntent {
    static let title: LocalizedStringResource = "Open Ultron Cockpit"
    static let description = IntentDescription("Åbn Ultron Cockpit-panelet (vejr, system, commute).")
    static let openAppWhenRun: Bool = true

    func perform() async throws -> some IntentResult {
        try await openUltron(action: "cockpit", prompt: "")
        return .result()
    }
}
...
struct OpenBriefingIntent: AppIntent {
    static let title: LocalizedStringResource = "Open Ultron Briefing"
    static let description = IntentDescription("Åbn Ultron Briefing-panelet (nyheder + historie).")
    static let openAppWhenRun: Bool = true

    func perform() async throws -> some IntentResult {
        try await openUltron(action: "briefing", prompt: "")
        return .result()
    }
}
...
struct DictateToClipboardIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<String> {
        ...
        try await openUltron(action: "dictate-clipboard", prompt: "")
...
struct AskWithContextIntent: AppIntent {
    static let title: LocalizedStringResource = "Ask Ultron with context"
    static let description = IntentDescription("Stil et spørgsmål om medfølgende tekst.")
    static let openAppWhenRun: Bool = true

    @Parameter(title: "Context", description: "Teksten Ultron skal arbejde med.")
...
/// Builds and opens a `ultron://` URL. AppIntents can't directly call into
...
@available(macOS 13.0, *)
private func openUltron(action: String, prompt: String) async throws {
    var components = URLComponents()
    components.scheme = "ultron"
...
struct UltronAppShortcuts: AppShortcutsProvider {
    static var appShortcuts: [AppShortcut] {
        AppShortcut(
            ...
            shortTitle: "Ask Ultron",
            systemImageName: "questionmark.circle"
        )
        AppShortcut(
            intent: OpenUltronChatIntent(),
            phrases: [
