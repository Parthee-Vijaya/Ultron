...
struct MessageBubble: View {
    ...
    private var userBubble: some View {
        MarkdownTextView(message.text, foregroundColor: .white)
            ...
            .padding(.vertical, 10)
            .background(UltronTheme.accent)
            .clipShape(RoundedRectangle(cornerRadius: 14, style: .continuous))
            .overlay(
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .stroke(UltronTheme.accentBright.opacity(0.6), lineWidth: 0.75)
            )
    }
    ...
    private var assistantBody: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            MarkdownTextView(message.text, foregroundColor: UltronTheme.textPrimary)
        ...
        Button { onRetry(message) } label: {
            ...
            .background(
                Capsule()
                    ...
                    .overlay(Capsule().stroke(UltronTheme.accent.opacity(0.5), lineWidth: 0.75))
            )
        }
        .buttonStyle(.plain)
        .padding(.top, 2)
...
private struct StreamingCursor: View {
    ...
    var body: some View {
        Text("▌")
            ...
            .accessibilityLabel("Ultron skriver")
