        ...
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                ...
                Button(action: copy) {
                    ...
                    .font(.caption)
                    .foregroundStyle(copied ? UltronTheme.accent : .secondary)
                }
...
private struct CitationChip: View {
    ...
    var body: some View {
        Button(action: open) {
            HStack(spacing: 5) {
                Text("\(citation.number)")
                    ...
                    .background(Circle().fill(UltronTheme.accent.opacity(hovering ? 1.0 : 0.85)))

                Text(label)
                    .font(.caption)
                    .lineLimit(1)
                    .foregroundStyle(hovering ? UltronTheme.textPrimary : .secondary)
            ...
            .overlay(
                Capsule(style: .continuous)
                    .stroke(UltronTheme.accent.opacity(hovering ? 0.5 : 0.2), lineWidth: 0.5)
            )
        ...
        .onHover { hovering = $0 }
        .animation(UltronTheme.springSnappy, value: hovering)
    }
