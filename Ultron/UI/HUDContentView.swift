...
struct HUDContentView: View {
    ...
    private func recordingView(elapsed: TimeInterval) -> some View {
            ...
            ZStack(alignment: .topLeading) {
                if speechService.transcript.isEmpty {
                    Text(audioLevel.isSilent ? "Slip for at sende" : "Lytter…")
                        .font(.system(size: 14))
                        .foregroundStyle(
                            audioLevel.isSilent ? UltronTheme.accent : UltronTheme.textSecondary
                        )
                        .transition(.opacity)
                } else {
        ...
        .animation(UltronTheme.springSnappy, value: audioLevel.isSilent)
        .animation(UltronTheme.springSnappy, value: speechService.transcript.isEmpty)
    }
    ...
    private var whisperBadge: some View {
        ...
        .background(
            Capsule(style: .continuous)
                .fill(UltronTheme.accent.opacity(0.15))
                .overlay(Capsule().stroke(UltronTheme.accent.opacity(0.4), lineWidth: 0.5))
        )
    ...
    private var processingView: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 10) {
                ProgressView()
                    .controlSize(.small)
                    .tint(UltronTheme.accent)
                Text(state.currentStep?.displayText ?? "Behandler…")
                    .font(.subheadline.weight(.semibold))
                    .foregroundStyle(UltronTheme.textPrimary)
                    .animation(UltronTheme.springSnappy, value: state.currentStep)
                Spacer()
                if let step = state.currentStep {
                    Image(systemName: step.icon)
                        .font(.system(size: 12, weight: .medium))
                        .foregroundStyle(UltronTheme.accent.opacity(0.75))
                        .transition(.opacity)
                }
            }
            if !speechService.transcript.isEmpty {
    ...
    private func resultView(text: String) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if text.count > 500 {
                ScrollView {
                    MarkdownTextView(text, foregroundColor: UltronTheme.textPrimary)
    ...
    private func confirmationView(message: String) -> some View {
        HStack(spacing: 10) {
            Image(systemName: "checkmark.circle.fill")
                .foregroundStyle(UltronTheme.successGlow)
            Text(message)
                .font(.system(size: 13))
                .foregroundStyle(UltronTheme.textPrimary)
            Spacer()
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 10) {
                Image(systemName: "lock.fill")
                    .foregroundStyle(UltronTheme.warningGlow)
    ...
    private func iconButton(system: String, active: Bool = false, help: String, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Image(systemName: system)
                ...
                .foregroundStyle(active ? UltronTheme.accent : UltronTheme.textSecondary)
