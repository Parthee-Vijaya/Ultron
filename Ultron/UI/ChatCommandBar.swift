...
struct ChatCommandBar: View {
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            .background(
                Capsule(style: .continuous)
                    .fill(UltronTheme.surfaceElevated.opacity(0.4))
                    .overlay(
                        Capsule(style: .continuous)
    ...
    private var placeholder: String {
        ...
        case .voice, .text:
            ...
            case "Agent":        return "Bed Ultron gøre noget…"
    ...
    private var plusMenu: some View {
        Menu {
            ...
            Image(systemName: "plus")
                ...
                .background(
                    Circle()
                        .fill(UltronTheme.surfaceElevated.opacity(0.9))
                )
        }
        .menuStyle(.borderlessButton)
        .menuIndicator(.hidden)
    ...
    private var micColor: Color {
        if isTranscribing { return UltronTheme.textMuted }
        ...
        return isRecording ? UltronTheme.criticalGlow : UltronTheme.textSecondary
    }
        ...
        case .screenshot:
            Button(action: performSubmit) {
                Image(systemName: "camera.viewfinder")
                    .font(.system(size: 20))
                    .foregroundStyle(canSubmitScreenshot ? UltronTheme.accent : UltronTheme.textMuted)
            }
            .buttonStyle(.plain)
        ...
        case .text, .voice:
            Button(action: performSubmit) {
                Image(systemName: "arrow.up.circle.fill")
                    .font(.system(size: 20))
                    .foregroundStyle(canSubmit ? UltronTheme.accent : UltronTheme.textMuted)
            }
            .buttonStyle(.plain)
            .disabled(!canSubmit)
