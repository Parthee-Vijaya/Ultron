...
struct ChatView: View {
    ...
    var body: some View {
        ...
        .animation(UltronTheme.springSnappy, value: showHistorySidebar)
    }
    ...
    private var dropHighlight: some View {
        if isDropTargeted {
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .stroke(UltronTheme.accent.opacity(0.7), lineWidth: 2)
                .padding(2)
                .allowsHitTesting(false)
        }
    }
    ...
    private func confirmationCard(_ pending: PendingToolCall) -> some View {
        ...
        .padding(12)
        .background(UltronTheme.surfaceElevated.opacity(0.7))
        .overlay(
            RoundedRectangle(cornerRadius: 8)
                .stroke(UltronTheme.accent.opacity(0.6), lineWidth: 1)
        )
        ...
        HStack(spacing: 10) {
            ...
            if !chatSession.messages.isEmpty {
                Button {
                    ...
                    .padding(.vertical, 4)
                    .background(
                        Capsule().fill(UltronTheme.surfaceElevated)
                    )
                }
                .buttonStyle(.plain)
        ...
        var body: some View {
            ...
            .background(
                Capsule(style: .continuous)
                    .fill(UltronTheme.surfaceElevated.opacity(0.5))
            )
            .overlay(
                Capsule(style: .continuous)
                    .stroke(UltronTheme.accent.opacity(0.15), lineWidth: 0.5)
            )
        ...
        ScrollViewReader { proxy in
            ScrollView {
                LazyVStack(spacing: 10) {
                    ...
                    // v1.4 Fase 2b: progress narration. Shows what Ultron is
            ...
            .onChange(of: chatSession.messages.count) {
                if let last = chatSession.messages.last {
                    withAnimation(UltronTheme.springSnappy) {
    ...
    private var emptyState: some View {
        ...
        return VStack(spacing: 0) {
            ...
            Text(pair.line)
                ...
                .foregroundStyle(UltronTheme.textPrimary.opacity(0.9))
    ...
    /// default nickname is just "P" — short, personal, Ultron-feel. Future:
