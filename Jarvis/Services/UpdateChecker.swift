...
final class UpdateChecker {
    private let repo = "Parthee-Vijaya/JarvisHUD"
    ...
    func checkNow(userInitiated: Bool) async {
        ...
        guard Self.compareSemver(remoteVersion, currentVersion) == .orderedDescending else {
            if userInitiated {
                showAlert(title: "Jarvis er opdateret", body: "v\(currentVersion) er den nyeste version.")
            }
            return
        }
    ...
    private func fetchLatestRelease() async -> Release? {
        ...
        request.setValue("Jarvis/\(Constants.appVersion)", forHTTPHeaderField: "User-Agent")
