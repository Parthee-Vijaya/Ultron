...
/// ⌘Space instead of opening Jarvis and scrolling the sidebar.
///
/// Each conversation becomes a `CSSearchableItem` with:
///   - `uniqueIdentifier`: the conversation UUID (lets us deep-link back).
///   - `domainIdentifier`: "jarvis.conversations" so one tap in Settings →
///     Spotlight can hide or scope just Jarvis's index.
...
///     index.
///   - `keywords`: mode-agnostic — just "Jarvis", "chat", and any unique
///     first-message words. Keeps results scoped.
///
/// Clicking a Spotlight hit fires a `jarvis://conversation?id=UUID` URL (the
...
final class SpotlightIndexer {
    ...
    private let domain = "jarvis.conversations"
    ...
    /// Nuclear reset — clears every conversation Jarvis has ever indexed.
    ...
    private static func keywords(for conversation: Conversation) -> [String] {
        var words: Set<String> = ["Jarvis", "chat", "samtale"]
