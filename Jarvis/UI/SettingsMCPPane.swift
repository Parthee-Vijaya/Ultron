...
/// Wraps the JSON editor at `~/.jarvis/mcp.json` in a proper form so users
...
struct SettingsMCPPane: View {
    ...
    var body: some View {
        SettingsPane(
            title: "MCP-servere",
            subtitle: "Model Context Protocol-servere eksponerer eksterne værktøjer til Agent-mode. Gemmes i ~/.jarvis/mcp.json."
        ) {
    ...
    private var serverListCard: some View {
        SettingsCard(
            ...
            footer: "Tryk på blyanten for at redigere en server inline. Ændringer skrives til ~/.jarvis/mcp.json og serverne genindlæses."
        ) {
            VStack(spacing: 0) {
