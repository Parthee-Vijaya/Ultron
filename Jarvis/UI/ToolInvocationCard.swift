...
struct ToolInvocationCard: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 8)
        .background(JarvisTheme.surfaceElevated.opacity(0.6), in: RoundedRectangle(cornerRadius: 10, style: .continuous))
        .overlay(
    ...
    private var header: some View {
        HStack(spacing: 8) {
            ...
            Image(systemName: invocation.success ? "checkmark.circle.fill" : "xmark.circle.fill")
                .font(.system(size: 11, weight: .semibold))
                .foregroundStyle(invocation.success ? JarvisTheme.accent : .red)
            Image(systemName: expanded ? "chevron.up" : "chevron.down")
                .font(.system(size: 9, weight: .semibold))
                .foregroundStyle(JarvisTheme.textSecondary.opacity(0.7))
        }
    }
    ...
    private func detailRow(label: String, body: String) -> some View {
        VStack(alignment: .leading, spacing: 2) {
            ...
            Text(body)
                ...
                .foregroundStyle(JarvisTheme.textPrimary.opacity(0.85))
    ...
    private var accentForStatus: Color {
        invocation.success ? JarvisTheme.accent : .red.opacity(0.85)
    }
